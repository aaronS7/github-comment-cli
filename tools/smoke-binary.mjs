...
assert.match(run('render', 'examples/review.md'), /github\.com\/aaronS7\/github-comment-cli\/blob\/[a-f0-9]{40}\/examples\/demo\.js#L3-L5/);

const temporary = await mkdtemp(path.join(tmpdir(), 'gh-comment-smoke-'));
try {
  const html = path.join(temporary, 'preview.html');
  assert.equal(run('preview', 'examples/review.md', '--output', html).trim(), html);
