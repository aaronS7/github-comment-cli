...
export class RepositoryError extends Error {
  constructor(message, code = 'REFERENCE_ERROR', options) {
...
export function parseLocalReference(target) {
...
async function runGit(cwd, args, { buffer = false } = {}) {
  const { stdout } = await execFileAsync('git', ['--literal-pathspecs', '-C', cwd, ...args], {
    encoding: buffer ? 'buffer' : 'utf8',
    maxBuffer: MAX_BUFFER,
    windowsHide: true,
  });
  return buffer ? stdout : stdout.trimEnd();
}
...
export class Repository {
  ...
  async resolveReference(target, { sha, repo } = {}) {
