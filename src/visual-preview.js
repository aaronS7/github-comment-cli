...
async function imageSources(attachments) {
  const sources = new Map();
  const trustedData = new Set();
  let total = 0;
...
function markdownRenderer({ sources, trustedData }) {
  ...
  return body => sanitizeHtml(marked.parse(body), {
...
function card(entry, index, renderMarkdown) {
...
export async function renderPreviewHtml(plan) {
  const renderMarkdown = markdownRenderer(await imageSources(plan.attachments));
  const review = plan.comments[0]?.kind === 'review';
  const cards = plan.comments.map((entry, index) => card(entry, index, renderMarkdown));
  const entries = review
    ? `<section class="review-group"><p class="group-heading">One ${escapeHtml(plan.comments[0].event)} review · ${plan.comments.length - 1} inline ${plan.comments.length === 2 ? 'thread' : 'threads'}</p>${cards.join('')}</section>`
    : cards.join('');
