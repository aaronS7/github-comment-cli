...
async function execute(command: string, args: string[], options: ExecFileOptions = {}) {
...
export function parseRepo(value: unknown, { serverUrl = DEFAULT_SERVER } = {}) {
...
export function parsePullRequest(value: string | number, { serverUrl = DEFAULT_SERVER } = {}) {
  if (/^[1-9]\d*$/.test(String(value))) return { number: positiveInteger(value, 'Pull request number') };
  let url;
  try { url = new URL(String(value)); } catch { throw new Error('Pull request must be a positive number or a GitHub pull request URL.'); }
...
export async function resolveToken({ env = process.env, run = execute, serverUrl = env.GITHUB_SERVER_URL || DEFAULT_SERVER }: {env?: NodeJS.ProcessEnv; run?: typeof execute; serverUrl?: string} = {}) {
...
export class GitHubError extends Error {
  ...
  method?: string;
  path?: string;
  constructor(message: string, { status, method, path }: {status?: number; method?: string; path?: string} = {}) {
    super(message);
...
export class GitHub {
  ...
  #attachmentRepositories = new Map<string, Promise<GitHubObject & {id: number}>>();

  constructor({ token, apiUrl = DEFAULT_API, fetch = globalThis.fetch, timeoutMs = 30_000 }: {token?: string; apiUrl?: string; fetch?: typeof globalThis.fetch; timeoutMs?: number} = {}) {
  ...
  async #request(method: string, path: string, body?: unknown) {
    ...
    try {
      ...
      throw new GitHubError(`GitHub ${method} request failed: ${this.#redact(errorMessage(error))}.${uncertainty}`, { method, path: url.pathname });
    }
    let data: unknown;
    ...
    if (!response.ok) {
      const detail = data && typeof data === 'object' && 'message' in data && typeof data.message === 'string' ? this.#redact(data.message) : 'Request failed';
  ...
  async getViewer() {
    try {
      const viewer = githubObject(await this.request('GET', '/user'));
      if (!Number.isSafeInteger(viewer?.id) || (viewer.id ?? 0) < 1 || !viewer?.login) throw new Error('GitHub did not return a valid authenticated user.');
      return { ...viewer, id: positiveInteger(viewer.id, 'User ID') };
    } catch (error) {
      ...
      const result = githubObject(await this.request('POST', api.href, { query: 'query { viewer { login databaseId } }' }));
      const viewer = result?.data?.viewer;
      if ((Array.isArray(result.errors) && result.errors.length) || !Number.isSafeInteger(viewer?.databaseId) || (viewer?.databaseId ?? 0) < 1 || !viewer?.login) {
      ...
      return { id: positiveInteger(viewer?.databaseId, 'User ID'), login: viewer?.login };
    }
  }

  async preflightAttachmentUpload(repo: string) {
    ...
    if (!this.#attachmentRepositories.has(key)) {
      const pending = (async () => {
        let repository;
        try { repository = githubObject(await this.request('GET', `/repos/${name}`)); }
        catch (error) {
        ...
        if (!Number.isSafeInteger(repository?.id) || (repository.id ?? 0) <= 0) {
        ...
        return Object.freeze({ ...repository, id: positiveInteger(repository.id, 'Repository ID'), permissions: Object.freeze({ ...repository.permissions }) });
      })();
  ...
  async uploadAttachment(repo: string, asset: { name: string; contentType: string; size: number; openBody: () => Promise<unknown> | unknown }) {
    ...
    url.search = new URLSearchParams({ name: asset.name, content_type: asset.contentType, repository_id: String(repository.id) }).toString();
    let body;
    ...
    try {
      ...
      try {
        response = await this.#fetch(url.href, {
          ...
          // Node fetch supports ReadStream bodies; its DOM declaration omits that extension.
          body: body as unknown as BodyInit, ...(body instanceof ReadStream ? { duplex: 'half' } : {}),
          redirect: 'error', signal: AbortSignal.timeout(this.timeoutMs),
        });
        raw = await response.text();
      } catch {
      ...
      try {
        ...
        const value = parsed && typeof parsed === 'object' && 'url' in parsed ? parsed.url : undefined;
        if (typeof value !== 'string') throw new Error();
        assetUrl = new URL(String(value));
  ...
  async paginate(path: string) {
    ...
    while (next) {
      ...
      items.push(...data.map(githubObject));
      next = headers.get('link')?.match(/<([^>]+)>;\s*rel="next"/)?.[1];
    }
    return items;
  }

  getPull(repo: string, number: number) { return this.request('GET', `/repos/${repo}/pulls/${positiveInteger(number, 'Pull request number')}`).then(githubObject); }
  async findPull(repo: string, head: string) {
  ...
  createComment(repo: string, number: number, body: string) { return this.request('POST', `/repos/${repo}/issues/${positiveInteger(number, 'Pull request number')}/comments`, { body }); }
  listComments(repo: string, number: number) { return this.paginate(`/repos/${repo}/issues/${positiveInteger(number, 'Pull request number')}/comments`); }
  updateComment(repo: string, id: number | undefined, body: string) { return this.request('PATCH', `/repos/${repo}/issues/comments/${positiveInteger(id, 'Comment ID')}`, { body }); }
  createReviewComment(repo: string, number: number, entry: Extract<CommentEntry, {kind: 'thread'}>, body: string, sha: string) {
  ...
  createFileComment(repo: string, number: number, entry: {path: string; kind?: 'file'; body?: string}, body: string, sha: string) {
    return this.request('POST', `/repos/${repo}/pulls/${positiveInteger(number, 'Pull request number')}/comments`, {
      body, commit_id: sha, path: entry.path, subject_type: 'file',
    });
  }
  createReviewReply(repo: string, number: number, parentId: number, body: string) {
  ...
  listReviewComments(repo: string, number: number) { return this.paginate(`/repos/${repo}/pulls/${positiveInteger(number, 'Pull request number')}/comments`); }
  createReview(repo: string, number: number, body: unknown) { return this.request('POST', `/repos/${repo}/pulls/${positiveInteger(number, 'Pull request number')}/reviews`, body); }
  listReviews(repo: string, number: number) { return this.paginate(`/repos/${repo}/pulls/${positiveInteger(number, 'Pull request number')}/reviews`); }
  listFiles(repo: string, number: number) { return this.paginate(`/repos/${repo}/pulls/${positiveInteger(number, 'Pull request number')}/files`); }
}

export type ContextGitHub = Partial<Pick<GitHub, 'getPull' | 'findPull'>>;

/** Resolve the destination PR and use its authoritative head commit for links. */
export async function resolveContext<T extends ContextGitHub = ContextGitHub>({ repo: explicitRepo, pr, cwd = process.cwd(), env = process.env, github: injectedGithub, run = execute, readFile = fsReadFile }: {repo?: string; pr?: string | number; cwd?: string; env?: NodeJS.ProcessEnv; github?: T; run?: typeof execute; readFile?: (path: string, encoding: 'utf8') => Promise<string>} = {}) {
  let github: T | GitHub | undefined = injectedGithub;
  ...
  let event: GitHubObject | undefined;
  if (env.GITHUB_EVENT_PATH && (!repo || !number)) {
    try { event = githubObject(JSON.parse(await readFile(env.GITHUB_EVENT_PATH, 'utf8'))); }
