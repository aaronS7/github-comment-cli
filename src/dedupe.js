...
function protectedWords(value, tokens) {
  const data = [];
...
function canonicalize(node, source, prose) {
  const exact = {};
  const protectedValue = {};
  for (const [key, value] of Object.entries(node)) {
    if (key === 'position') continue;
    if (key === 'children') {
      const children = value.map(child => canonicalize(child, source, prose));
...
function asFingerprint(body) {
  return body?.[FINGERPRINT] ? body : fingerprint(body);
}
 ...
 */
export function compareBodies(left, right, { mode = 'exact', threshold = 0.96 } = {}) {
