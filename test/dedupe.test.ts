...
const similar = (left: string, right: string, threshold = 0.96) => compareBodies(left, right, { mode: 'similar', threshold });
...
test('fuzzy matching is opt-in and uses cached fingerprints interchangeably', () => {
  ...
  if (match === null) assert.fail('Expected a similar match');
...
test('validates the pure comparison API options and body types', () => {
  for (const threshold of [0, -1, 1.1, NaN, Infinity, '0.96']) {
    assert.throws(() => compareBodies('a', 'a', { threshold } as unknown as Parameters<typeof compareBodies>[2]), /Similarity threshold/);
  }
  assert.throws(() => compareBodies('a', 'a', { mode: 'guess' } as unknown as Parameters<typeof compareBodies>[2]), /mode must be exact or similar/);
