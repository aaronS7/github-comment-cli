...
test('review, file, and reply directives keep placement separate from Markdown bodies', async () => {
  ...
  for (const event of ['APPROVE', 'REQUEST_CHANGES']) {
    const [entry] = await renderMarkdown(`<!-- gh-comment:review event="${event}" -->\nSummary.`, fixedResolver);
    assert.equal(entry?.kind, 'review');
    if (entry?.kind === 'review') assert.equal(entry.event, event);
  }
});
...
test('integrates real Git snapshots, absolute paths, URL encoding, and dirty-line failures', async t => {
  ...
  const resolve = (target: string) => repository.resolveReference(target, { sha, repo: 'example/project' });
