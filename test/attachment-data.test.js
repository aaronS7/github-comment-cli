...
import { isPublicAddress, prepareAttachment } from '../src/attachment-data.js';

const PNG = Buffer.concat([Buffer.from([137, 80, 78, 71, 13, 10, 26, 10]), Buffer.from('image fixture bytes')]);
const digest = bytes => createHash('sha256').update(bytes).digest('hex');
const remote = { downloadRemote: true, allowPrivateNetwork: true };
...
async function server(t, handler) {
  ...
  return { base: `http://127.0.0.1:${listener.address().port}`, port: listener.address().port };
}

async function body(prepared) {
...
test('spills into private disk snapshots, supports file URLs, and removes snapshots on disposal', async t => {
  ...
  const snapshotPath = stream.path;
  stream.destroy();
...
test('supports SVG and generic binary responses only when remote media bytes match', async t => {
  ...
  const { base } = await server(t, (req, res) => {
    res.writeHead(200, { 'Content-Type': req.url.endsWith('.svg') ? 'image/svg+xml' : 'application/octet-stream' });
    res.end(req.url.endsWith('.svg') ? svg : PNG);
  });
...
test('redirects are bounded, cookies are not forwarded, and private redirect targets are rechecked', async t => {
  ...
  await assert.rejects(prepareAttachment('http://public.invalid/start.png', {
    ...
    requestImpl: async () => {
      hops += 1;
      return Object.assign(Readable.from([]), { statusCode: 302, headers: { location: 'http://127.0.0.1/private.png' } });
    },
  }), { code: 'PRIVATE_NETWORK' });
  assert.equal(hops, 1);
});
...
test('rejects URL credentials and does not expose query strings in failed downloads', async t => {
  await assert.rejects(prepareAttachment('https://user:password@example.com/image.png?token=secret', remote), error => error.code === 'URL_CREDENTIALS' && !/password|secret/.test(error.message));
  const { base } = await server(t, (_req, res) => { res.writeHead(403); res.end('secret server body'); });
  await assert.rejects(prepareAttachment(`${base}/image.png?token=secret`, remote), error => error.code === 'HTTP_ERROR' && !/secret|token=/.test(error.message));
});
...
test('decompresses gzip, deflate, and Brotli while limiting decoded bytes', async t => {
  ...
  const { base } = await server(t, (req, res) => {
    const [encoding, kind] = req.url.slice(1).split('/');
...
test('rejects lying body lengths and cleans a completed disk snapshot on protocol failure', async t => {
  ...
  await assert.rejects(prepareAttachment('http://public.invalid/image.png', {
    ...
    lookup: async () => [{ address: '8.8.8.8', family: 4 }],
    requestImpl: async () => Object.assign(Readable.from([PNG]), {
      statusCode: 200, headers: { 'content-type': 'image/png', 'content-length': String(PNG.length - 1) },
    }),
...
test('validates options and rejects unknown extension or extensionless unknown MIME', async t => {
  for (const options of [{ memoryLimitBytes: -1 }, { maxBytes: 0 }, { timeoutMs: 0 }, { maxRedirects: -1 }, { downloadRemote: 'false' }, { allowPrivateNetwork: 'false' }]) {
    await assert.rejects(prepareAttachment('image.png', options), { code: 'INVALID_OPTION' });
  }
