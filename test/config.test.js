...
async function fixture(t, { git = false } = {}) {
  ...
  return {
    root,
    write: (value, name = CONFIG_FILENAME) => writeFile(path.join(root, name), JSON.stringify(value)),
  };
}
...
test('rejects malformed JSON and unknown settings without echoing secret values', async t => {
  ...
  await assert.rejects(loadConfig({ cwd: root }), error => /not valid JSON/.test(error.message) && !error.message.includes('private-secret'));
  await write({ token: 'private-secret' });
  await assert.rejects(loadConfig({ cwd: root }), error => /Unknown setting "token"/.test(error.message) && !error.message.includes('private-secret'));
});
