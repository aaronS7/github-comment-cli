...
const ASSET: AssetMetadata = { sha256: 'a'.repeat(64), contentType: 'image/png', url: FIRST };
const marker = (payload: unknown) => `<!-- gh-comment:attachments:${Buffer.from(JSON.stringify(payload)).toString('base64url')} -->`;
const rewrite = (source: string, url = FIRST, video = false) => rewriteAttachments(source, attachmentReferences(source)
...
test('canonicalizing a video also rewrites its now-unused reference definition', () => {
  const firstAsset: AssetMetadata = { ...ASSET, contentType: 'video/mp4' };
