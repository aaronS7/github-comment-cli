...
import type { CommentEntry, ReviewEvent } from '../src/types.js';
import type { PublicationPlan } from '../src/publication-types.js';
import type { AssetMetadata } from '../src/attachment-types.js';

const SHA = 'a'.repeat(40);
const VIEWER = { id: 42, login: 'reviewer' };
const commentUrl = (id: number) => `https://github.com/example/project/pull/12#issuecomment-${id}`;

type RemoteComment = Record<string, unknown> & { id: number; body: string; user?: { id: number; login?: string }; html_url?: string };
type Call = { method: string; [key: string]: unknown };
type ReviewCommentPayload = { path: string; line: number; side: 'LEFT' | 'RIGHT'; body: string; start_line?: number };
type ReviewPayload = { body: string; event: ReviewEvent; commit_id: string; comments: ReviewCommentPayload[] };
type FixtureOptions = {
  comments?: RemoteComment[]; reviewComments?: RemoteComment[]; reviews?: RemoteComment[];
  bodies?: string[]; settings?: Partial<NonNullable<PublicationPlan['settings']>>;
  marker?: string; overrides?: Partial<PublicationPlan['context']['github']>;
};

function fixture({ comments = [], reviewComments = [], reviews = [], bodies = ['Report'], settings, marker, overrides = {} }: FixtureOptions = {}) {
  ...
  const github = {
    ...
    async listComments(repo: string, pr: number) { calls.push({ method: 'listComments', repo, pr }); return remote.map(comment => ({ ...comment })); },
    async listReviewComments(repo: string, pr: number) { calls.push({ method: 'listReviewComments', repo, pr }); return remoteReview.map(comment => ({ ...comment })); },
    async listReviews(repo: string, pr: number) { calls.push({ method: 'listReviews', repo, pr }); return remoteReviews.map(review => ({ ...review })); },
    async getPull(repo: string, pr: number) { calls.push({ method: 'getPull', repo, pr }); return { number: pr, head: { sha: SHA } }; },
    ...
    async createReviewComment(repo: string, pr: number, entry: Extract<CommentEntry, { kind: 'thread' }>, body: string, sha: string) {
    ...
    async createFileComment(repo: string, pr: number, entry: Extract<CommentEntry, { kind: 'file' }>, body: string, sha: string) {
    ...
    async createReviewReply(repo: string, pr: number, parentId: number, body: string) {
      calls.push({ method: 'createReviewReply', repo, pr, parentId, body });
      const comment = { ...own(body, nextId++), in_reply_to_id: parentId };
      remoteReview.push(comment);
      return { ...comment };
    },
    async createReview(repo: string, pr: number, body: ReviewPayload) {
    ...
    async preflightAttachmentUpload(): Promise<never> { throw new Error('Unexpected attachment preflight'); },
    async uploadAttachment(): Promise<never> { throw new Error('Unexpected attachment upload'); },
    ...overrides,
  };
  const plan: PublicationPlan = { repo: 'example/project', pr: 12, sha: SHA, context: { github },
...
function thread(body: string, line = 12): CommentEntry {
  return { kind: 'thread', path: 'src/file.js', side: 'RIGHT', startLine: line, line, body };
}

function batch(body = 'Review summary', event: ReviewEvent = 'COMMENT', threads: CommentEntry[] = [thread('Check this line')]): CommentEntry[] {
  return [{ kind: 'review', event, body }, ...threads];
}

function submittedReviewComments(call: Call): ReviewCommentPayload[] {
  const body = call.body;
  assert.ok(body && typeof body === 'object' && 'comments' in body);
  assert.ok(Array.isArray(body.comments));
  return body.comments as ReviewCommentPayload[];
}

function ownReview(body: string, line = 12, id = 200): RemoteComment {
  return { ...own(body, id), path: 'src/file.js', side: 'RIGHT', start_line: line, line, position: 1 };
}

function own(body: string, id = 100): RemoteComment {
...
test('file comments and replies deduplicate only at their own placement', async () => {
  ...
  assert.equal((result.comments[3] as { parentId?: number }).parentId, 201);
});
...
test('a batch review submits only new threads in one API request and reruns cleanly', async () => {
  ...
  assert.equal(submittedReviewComments(f.writes()[0]).length, 1);
  assert.equal(submittedReviewComments(f.writes()[0])[0].line, 13);
...
test('a submitted review is visible in partial results if comment URL lookup fails', async () => {
  ...
  await assert.rejects(publish(f.plan), error => {
    const failure = error as Error & { partialResult: { comments: { action: string }[] } };
    assert.match(failure.message, /was submitted/);
    assert.equal(failure.partialResult.comments.length, 1);
    assert.equal(failure.partialResult.comments[0].action, 'created');
    return true;
  });
  assert.equal(f.writes().length, 1);
...
test('active attachments load both comment types even for a single kind of entry', async () => {
  ...
  const asset = { sha256: 'a'.repeat(64), contentType: 'image/png' as const, url };
  ...
  ] as [CommentEntry[], RemoteComment[], RemoteComment[]][]) {
    const f = fixture({ comments, reviewComments, bodies: [], settings: { dedupe: 'off' } });
    f.plan.comments = entries;
    let cache: Map<string, AssetMetadata> | undefined;
    f.plan.attachments = {
      active: true,
      setCache(value: Map<string, AssetMetadata>) { cache = value; },
      validate() {},
      hasUploads() { return false; },
      describe() { return []; },
    } as unknown as NonNullable<PublicationPlan['attachments']>;
...
test('partial failure reports skipped and written entries separately and a rerun resumes safely', async () => {
  ...
  await assert.rejects(publish(f.plan), error => {
    const failure = error as Error & { partialResult: { comments: { action: string; url?: string }[] } };
    assert.match(failure.message, /Comment 3/);
    assert.match(failure.message, /Published 1 of 4/);
    assert.deepEqual(failure.partialResult.comments.map(comment => comment.action), ['skipped', 'created']);
    assert.equal(failure.partialResult.comments[0].url, commentUrl(100));
    assert.equal(failure.partialResult.comments[1].url, commentUrl(1000));
    return true;
  });
