...
function fixture({ token = TOKEN, repository = REPOSITORY, upload, apiUrl } = {}) {
  const requests = [];
...
function asset(overrides = {}) {
  ...
  return { name: 'screen shot.png', contentType: 'image/png', size: bytes.length, openBody: () => bytes, ...overrides };
}

async function fileAsset(t, overrides = {}) {
...
test('attachment preflight verifies and caches immutable repository authorization', async () => {
  ...
  assert.throws(() => { first.permissions.push = false; });
});
...
test('file streams upload with duplex support and close before returning', async t => {
  ...
  assert.equal(file.stream().destroyed, true);
});
...
test('a response-body failure closes the source stream and keeps the outcome ambiguous', async t => {
  const file = await fileAsset(t);
  const f = fixture({ upload: async () => ({ text: async () => { throw new Error('signed-private-response'); } }) });
...
test('invalid metadata and changed buffer sizes fail without uploads', async () => {
  for (const value of [asset({ name: '../file.png' }), asset({ name: 'file\n.png' }), asset({ size: 0 }),
    asset({ contentType: 'image/png\r\nInjected: true' }), asset({ openBody: undefined })]) {
    const f = fixture();
