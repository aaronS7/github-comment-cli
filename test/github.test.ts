...
type ContextClient = Partial<Pick<GitHub, 'getPull' | 'findPull'>>;
...
test('timeline publication sends exact Markdown JSON with authentication', async () => {
  ...
  const github = new GitHub({ token: 'test-token', fetch: async (url, options = {}) => {
    ...
    assert.deepEqual(JSON.parse(requestBody(options)), { body: '# Comment\n\n`$()` stays literal.' });
    return json({ id: 7, html_url: 'https://github.com/owner/project/pull/42#issuecomment-7' }, 201);
  } });
  const created = await github.createComment('owner/project', 42, '# Comment\n\n`$()` stays literal.');
  assert.ok(created && typeof created === 'object' && 'id' in created);
...
test('new PR review operations send the documented endpoint fields', async () => {
  ...
  const github = new GitHub({ token: 'test-token', fetch: async (url, options = {}) => {
    seen.push({ path: new URL(requestUrl(url)).pathname, method: options.method, body: options.body ? JSON.parse(requestBody(options)) : undefined });
  ...
  await github.createFileComment('owner/project', 42, { kind: 'file', path: 'src/file.js', body: 'Whole file.' }, 'Whole file.', sha);
...
test('an interrupted response body is a sanitized ambiguous write failure', async () => {
  const github = new GitHub({ token: 'test-token', fetch: async () => ({ text: async () => { throw new Error('stream failed test-token'); } } as unknown as Response) });
  await assert.rejects(github.createComment('owner/project', 42, 'Hello'), error => {
    assert.ok(error instanceof Error);
    assert.match(error.message, /may have reached GitHub/);
    assert.equal(error.message.includes('test-token'), false);
...
test('viewer identity falls back to GraphQL for installation tokens', async () => {
  ...
  const github = new GitHub({ token: 'test-token', fetch: async (url, options = {}) => {
    ...
    seen.push(href);
    if (href.endsWith('/user')) return json({ message: 'Resource not accessible by integration' }, 403);
    assert.equal(href, 'https://api.github.com/graphql');
    assert.deepEqual(JSON.parse(requestBody(options)), { query: 'query { viewer { login databaseId } }' });
...
test('viewer fallback fails closed when API cannot establish identity', async () => {
  const github = new GitHub({ token: 'test-token', fetch: async (url) => requestUrl(url).endsWith('/user')
...
test('resolveContext accepts an explicit URL without local git or an Actions event', async () => {
  const github: ContextClient = { getPull: async (repo: string, number: number) => {
    assert.equal(repo, 'owner/project');
    assert.equal(number, 42);
    return pull;
  } };
  const context = await resolveContext<ContextClient>({ pr: 'https://github.com/owner/project/pull/42', env: {}, run: noRun, github });
...
test('Actions resolve PR number and base repo while head SHA comes from the API', async () => {
  const github: ContextClient = { getPull: async (repo: string, number: number) => { assert.equal(repo, 'owner/project'); assert.equal(number, 42); return pull; } };
  const context = await resolveContext<ContextClient>({
    ...
    run: noRun,
    readFile: async (_path, _encoding) => JSON.stringify({ number: 42, pull_request: { number: 42, head: { sha: 'stale-event-sha' }, base: { repo: { full_name: 'owner/project' } } } }),
    github,
  });
  assert.equal(context.headSha, sha);
});
...
test('issue_comment Actions event only selects an issue that is a PR', async () => {
  const context = await resolveContext({
    ...
    readFile: async (_path, _encoding) => JSON.stringify({ issue: { number: 42, pull_request: {} } }),
...
test('branch discovery supports fork origin with upstream destination', async () => {
  ...
  const context = await resolveContext<ContextClient>({ env: {}, run, github: {
...
// GitHub's REST schema permits these fields to be null, including responses
// used by duplicate detection and reply-parent validation.
test('review lists preserve nullable review and commit identifiers', async () => {
  const comment = { id: 1, body: 'Inline comment', pull_request_review_id: null };
  const review = { id: 2, body: '', commit_id: null };
  const github = new GitHub({ fetch: async url => {
    const pathname = new URL(requestUrl(url)).pathname;
    return json(pathname.endsWith('/reviews') ? [review] : [comment]);
  } });
  assert.deepEqual(await github.listReviewComments('owner/project', 42), [comment]);
  assert.deepEqual(await github.listReviews('owner/project', 42), [review]);
});
