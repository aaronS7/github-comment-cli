...
function parseManagedValue(contents, name) {
...
async function saveConfig({ clientId, keyPath, repository }) {
  let existing = '';
...
function validateConfig(body) {
  ...
  const keyPathInput = String(body.keyPath ?? '').trim();
...
async function startServer() {
  ...
  const handleRequest = async (request, response) => {
    ...
    try {
      ...
      sendJson(response, 400, { error: error.message || 'Could not process the request.' });
    }
  };
      ...
      await new Promise((resolve, reject) => {
        ...
        server.listen(port, listenHost, () => {
          ...
          server.on('error', error => {
            process.stderr.write(`GitHub App setup server: ${error.message}\n`);
            process.exitCode = 1;
          });
          resolve();
        });
...
if (process.argv.includes('--show-code')) {
  readAccessCode().then(code => process.stdout.write(`${code}\n`)).catch(error => {
    process.stderr.write(`GitHub App setup code: ${error.message}\n`);
    process.exitCode = 1;
  });
} else {
