...
type RepositoryName = { owner: string; repo: string; fullName: string };
type AppInputs = { clientId: string; keyPath: string; repository: RepositoryName };

function hasCode(error: unknown, code: string): boolean {
  return typeof error === 'object' && error !== null && 'code' in error && error.code === code;
}

function valueFromEnvLine(line: string, name: string): string | undefined {
...
function repoFromPullRequest(value: string | undefined): string | undefined {
...
function inferredRepo(args: string[]): string | undefined {
...
function validateRepo(value: string): RepositoryName {
...
async function promptValue(rl: Interface, label: string, defaultValue?: string): Promise<string> {
...
async function collectInputs(args: string[]): Promise<AppInputs> {
...
async function mintInstallationToken({ clientId, keyPath, repository }: AppInputs): Promise<string> {
...
function runGhComment(args: string[], token: string, repository: RepositoryName): Promise<number> {
...
main().catch((error: unknown) => {
  process.stderr.write(`gh-comment App auth: ${error instanceof Error ? error.message : String(error)}\n`);
  process.exitCode = 1;
});
